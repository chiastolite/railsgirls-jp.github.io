...
We're going to use [**Sinatra**](https://www.sinatrarb.com/) as a tool to demonstrate some basic web principles. Sinatra is a small framework for creating web applications in Ruby with minimal effort. You can find a list of [things built with Sinatra here](https://www.sinatrarb.com/wild.html).
...
If you get stuck, make sure your app.rb looks like [this one](https://tjmcewan.github.io/coffeecalc/snippets/install_sinatra.rb.txt).
...
If it's not quite working, make sure your code looks like [this code](https://tjmcewan.github.io/coffeecalc/snippets/html_form.rb.txt).
...
For simplicity, let's just store the coffee details in a variable. So that they'll be available between requests, we'll need to use a [**global**](https://en.wikipedia.org/wiki/Global_variable) variable. This is a drastically simplified version of what Rails calls the Model layer.
...
**Hint:**
Your global variable is an array, so if you get stuck, try [Ruby's Array documentation ](https://www.ruby-doc.org/core-2.1.0/Array.html#method-i-3C-3C) to find out how you can add information to an array.  
